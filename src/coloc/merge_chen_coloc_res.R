...
full_list = list.files('results/coloc_res/chen/full', full.names = T)
...
# Add compound name:
compound_info = read.table('data/chen_metqtl/PMID36635386_studies_export.tsv', sep = '\t', header = T, stringsAsFactors = F)
compound_info = compound_info %>% mutate(phenostring = gsub(' levels', '', reportedTrait)) %>% select(accessionId, phenostring) %>% rename(phenocode = accessionId)

res_list = res_list %>% left_join(., compound_info, by = c('compound' = 'phenocode')) %>% select(locus:compound, phenostring, SNP:PP.H4.abf)
...
per_comp = per_comp %>% select(phenocode, phenostring, cohort.coloc, count.locus:coloc_variants)

# Save a bunch of files
write.table(res_list, 'results/coloc_res/chen/merged_res/coloc_merged.txt', sep = '\t', col.names = T, row.names = F, quote = F) # NOTE: this file isn't filtered for matching coloc variant and locus cohort
write.table(res_sig, 'results/coloc_res/chen/merged_res/coloc_merged.pp0.8.txt', sep = '\t', col.names = T, row.names = F, quote = F)
write.table(per_comp, 'results/coloc_res/chen/merged_res/per_compound_summary.txt', sep = '\t', col.names = T, row.names = F, quote = F)
write.table(loci_count_met, 'results/coloc_res/chen/merged_res/per_locus_summary.txt', sep = '\t', col.names = T, row.names = F, quote = F)
