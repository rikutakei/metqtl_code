...
# input
#
# Rscript src/coloc/run_coloc.metsim.R C98 full
...
metabolite = args[1]
met_dir = paste('results/metqtl/metsim/', metabolite, sep = '')
...
met_list = map(names(common_snps), ~ met_list[[.x]] %>% filter(rsids %in% common_snps[[.x]]))
...
run_coloc = function(gwas_dat, met_dat, prop_case = 0.0478) {
	...
	met = list(snp = met_dat$rsids,
			   beta = met_dat$beta,
			   MAF = met_dat$maf,
			   N = 6136,
			   varbeta = (met_dat$sebeta ^ 2),
			   pvalues = met_dat$pval,
			   type = 'quant')
...
# Save result
out_name = paste('results/coloc_res/metsim', cohort, metabolite, sep = '/')
