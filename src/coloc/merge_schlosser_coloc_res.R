...
type = args[1]
full_list = gsub('type', type, 'results/coloc_res/schlosser/type/full')
full_list = list.files(full_list, full.names = T)
...
# Add compound name:
compound_info = read.table('data/schlosser_metqtl/PMID37277652_studies_export.tsv', sep = '\t', header = T, stringsAsFactors = F)
compound_info = compound_info %>% select(reportedTrait, summaryStatistics)
compound_info = compound_info %>% transmute(phenostring = gsub(' levels in chronic kidney disease', '', reportedTrait), phenostring = gsub(paste(str_to_title(type), ' ', sep = ''), '', phenostring), compound = gsub('.*/', '', summaryStatistics))

res_list = res_list %>% left_join(., compound_info, by = c('compound')) %>% select(locus:compound, phenostring, SNP:PP.H4.abf)
res_list$category = type
...
per_comp = per_comp %>% select(phenostring, cohort.coloc, count.locus:coloc_variants)

# Save a bunch of files
write.table(res_list, gsub('type', type, 'results/coloc_res/schlosser/type/merged_res/coloc_merged.type.txt'), sep = '\t', col.names = T, row.names = F, quote = F) # NOTE: this file isn't filtered for matching coloc variant and locus cohort
write.table(res_sig, gsub('type', type, 'results/coloc_res/schlosser/type/merged_res/coloc_merged.pp0.8.type.txt'), sep = '\t', col.names = T, row.names = F, quote = F)
write.table(per_comp, gsub('type', type, 'results/coloc_res/schlosser/type/merged_res/per_compound_summary.type.txt'), sep = '\t', col.names = T, row.names = F, quote = F)
write.table(loci_count_met, gsub('type', type, 'results/coloc_res/schlosser/type/merged_res/per_locus_summary.type.txt'), sep = '\t', col.names = T, row.names = F, quote = F)
